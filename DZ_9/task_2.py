    ...
    while True:
        ...
        if not quantity_ips.isnumeric():
            ...
            if (last_octet + int(quantity_ips)) > 254:
                print(f"Возможное число проверки не больше: {254 - last_octet}")
            else:
                break

    ip_list = []
    [ip_list.append(str(ip_address(begin_ip) + x)) for x in range(int(quantity_ips))]
